...
namespace Geek.DynamicJSProxies.Web.Api.ProxyScripting.Generators
{
    public static class ProxyScriptingHelper
    {
        ...
        private static string AddQueryStringParameters(string url, IList<ParameterApiDescriptionModel> actionParameters)
        {
            var queryStringParameters = actionParameters
                .Where(p => new string[] { ParameterBindingSources.ModelBinding, ParameterBindingSources.Query}.Contains(p.BindingSourceId))
                .ToArray();
            ...
            var qsBuilderParams = string.Join(", ", queryStringParameters.Select(p => $"{{ name: '{p.Name.ToCamelCase()}', value: {ProxyScriptingJsFuncHelper.GetParamNameInJsFunc(p)} }}"));
